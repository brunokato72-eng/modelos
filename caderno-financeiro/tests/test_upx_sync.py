...
class TestSincronizar(unittest.TestCase):
    ...
    def test_counterparty_titular_sem_par_visivel_e_mantida(self):
        """`counterparty_name` = o próprio titular NÃO é, sozinho, motivo pra
        descartar: é o formato normal de uma portabilidade de salário (conta-
        salário do empregador redireciona automaticamente pra essa conta —
        aparece com o titular como contraparte mesmo sendo dinheiro novo).
        Só descarta quando há um PAR visível (mesma leva, mesma data/valor,
        uma entrada e uma saída) confirmando que é o mesmo dinheiro se
        movendo — ver `test_transferencia_entre_contas_proprias_...`."""
        transacoes = [pagina([
            transacao_bruta(
                transaction_id="tx-portabilidade-1", valor=3636.0, direction="inflow",
